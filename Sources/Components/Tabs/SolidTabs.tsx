...
import { SolidTabsProps, SolidTabItem } from '@Types'
...
class SolidTabs extends React.Component<Props, State> {
  ...
  private onLayout = (e: LayoutChangeEvent) => {
    ...
    const borderWidth =
      (style && StyleSheet.flatten(style).borderRadius) || styles.container.borderWidth
  ...
  private renderItem = (item: SolidTabItem, index: number, length: number, selected: boolean) => {
    const {
      textStyle,
