...
import CodePush, { RemotePackage, DownloadProgress, CodePushOptions } from 'react-native-code-push'
...
const productionKey_iOS = 'fSrHIO8eBJPVAf5UFOT82WyB-33pByKRuiPD4'
const stagingKey_iOS = 'ogttyZl74GoSIkC5pTGkvM4OtQrGS1UpdsPwN'

const productionKey_android = 'afg3u_8Yme8TkM4OkGODRMGkNccSHJi1tsvDE'
const stagingKey_android =
  'JGGs3JrbMSjDVMz5_7769UYG9YyUr1IkKoDwEe90xvsrSJHJZM9EPaOfTCifnXeNEr1M4d-LPN'
...
class CodePushUpdate extends React.Component<Props, State> {
  private titles: StateKey = {
    None: Strings.updateAvailable,
    Syncing: Strings.updateInProgress,
    Update: Strings.updateAvailable,
    Updated: Strings.updateInstalled,
  }
  ...
  private close = (onClose?: () => void) => {
    ...
    ]).start(() => this.setState({ state: 'None' }, onClose))
  }
  ...
  private syncImmediate() {
    CodePush.checkForUpdate(this.props.deploymentKey).then(update => {
      if (update) {
        ...
        this.setState({ isMandatory, updateInfo: update, state: 'Update' }, this.show)
      }
    })
  }
      ...
      return (
        <View style={{ alignItems: 'center' }}>
          <View style={styles.progressBar}>
            ...
            <Animated.Text style={[styles.progress, { color }]}>{progress}</Animated.Text>
          </View>
  ...
  private renderContent = () => {
    ...
    if (state === 'Updated') {
      return (
        <View style={styles.content}>
          <Text style={styles.header} text={Strings.newestInstalled} />
          <Text style={styles.confirmText} text={Strings.doYouWantUpdateNow} />
        </View>
      )
    }
    ...
    return (
      <View style={styles.content}>
        <Text style={styles.header} text={Strings.newFeatures} />
        {this.renderDescription()}
        <Text
          style={styles.confirmText}
          text={isMandatory ? Strings.pleaseUpdate : Strings.doYouWant}
        />
      </View>
    )
  }
